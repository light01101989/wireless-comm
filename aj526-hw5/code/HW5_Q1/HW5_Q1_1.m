...
function [BER,legendtext] = HW5_Q1_1(splot)
...
symIn = qammod(consIn,M,0,'gray');

ofdmsymIn = reshape(symIn,length(symIn)/nSub,nSub);
...
for k = 1:length(sd)
  ...
  %% Receiver
  %y = y_w./lamda;
  y = (conj(lamda).*y_w)./(abs(lamda));

  y_temp = reshape(y,1,T*nSub);
  x_temp = nn_detector(M,y_temp);

  x_hat = reshape(x_temp,T,nSub);

  %% STEP 5 OF M-C SIMULATION
  %% Calculating BER
  %[~, decoded_sym] = ismember(x_hat,baseline_symbols(M));
  decoded_sym = qamdemod(x_hat,M,0,'gray');
  decoded_sym = reshape(decoded_sym,T*nSub,1);
