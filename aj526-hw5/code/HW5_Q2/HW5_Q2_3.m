...
%% Email/Netid: aj526
%% Date: 11/5/2015
%% OFDM simulation BICM-OFDM without interleaving

function [BER,legendtext] = HW5_Q2_3(SNR_db,splot)
% Simulate the BICM-OFDM case with 1/2 coding rate and no interleaving
%
% Input:
...
%% interleaving
%intlv = [(1:Nbits*2)(1:2:end) (1:Nbits*2)(2:2:end)];
...
for k = 1:length(sd)
  ...
  %% Decoder
  %% D_mat(Tx64)
  D_mat = l1 + l2;  %llr(:,1:nSub) + llr(:,nSub+1:end);
...
%%PLOTTING
legendtext = strcat('OFDM QPSK: BICM wo interleaving');
if splot ~= 0
    figure(1)
