function [par,idxhat,bithat,i,minMlcost] = mgs_mr(par,H,y,N0)

    Rmax = 50;
    H_temp = [real(H) -imag(H);imag(H) real(H)];
    y_temp = [real(y);imag(y)];
    offset = y'*y;
    par.comp = par.comp + par.MR;
    minMlcost = Inf;
    repcnt = 0;
    solvec = [];
    c2 = 0.5*par.Q;
    ...
    while i<=Rmax
        ...
        [par,~,~,z(:,i),beta(i)] = mgs(par,H,y,x0,N0);

        if beta(i)<minMlcost
            minMlcost = beta(i);
            solvec = z(:,i);
            [par,standMLcost] = standMLcostFn(solvec,H_temp,y_temp,par,offset,N0);
...
function [par,standMLcost] = standMLcostFn(x,H,y,par,offset,N0)
    cost = x'*H'*H*x - 2*y'*H*x + offset;
    par.comp = par.comp + 2*((2*par.MR)*(2*par.MT))+ 2*par.MR + (2*par.MR)*(2*par.MT) + 2*par.MR;
