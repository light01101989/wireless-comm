function [par,idxhat,bithat,z,beta] = mgs(par,H,y,x0,N0)
...
%     MAXITER = 256;
    ...
    q = 1/(2*par.MT);
    [par,beta] = costFn(par,x0,H,y,offset);
    z = x0;
    x1 = x0;
    t = 1;
    betavec(t) = beta;
    ...
    while t <= MAXITER
        for i = 1:2*par.MT
            ...
            if (rk > q)
                for r = 1:length(rsym)
                    x1(i) = rsym(r);
                    [par,costvec(r)] = costFn(par,x1,H,y,offset);
                end
                [~,idx] = min(costvec);
                x1(i) = rsym(idx);
            else
        ...
        [par,gamma] = costFn(par,x1,H,y,offset);
        cf(t) = gamma;
        if (gamma <= beta)
            z = x1;
...
function [par,cost] = costFn(par,x,H,y,offset)
    cost = x'*H'*H*x - 2*y'*H*x + offset;
    par.comp = par.comp + ((2*par.MT)*(2*par.MT))+ 2*par.MT + 2*par.MT;
end

function [theta] = stallFn(x,H,y,par,offset,N0)
    c1=10*par.Q;
    %standMLcost = (costFn(x,H,y,offset) - par.MR*(par.sigmak).^2)/(sqrt(par.MR)*(par.sigmak).^2);
    [~,CF] = costFn(par,x,H,y,offset);
    standMLcost = (CF - par.MR*N0)/(sqrt(par.MR)*N0);
