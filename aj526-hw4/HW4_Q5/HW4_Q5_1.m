...
%% Detection performed with joint MAP channel estimates

%clear all;close all;
function [SER legendtext] = HW4_Q5_1(L,splot)
...
for p = 1:length(L)
    for k = 1:length(sd)
      ...
      argm=[-1,1];
      for i=1:length(argm)
        h_hat = (y_1+argm(i).*y_2)/repmat((var(1,k)+1+(argm(i).^2)'),1,T);

        %% Detection Rule
        %Y = [y_1,y_2];
        temp1 = (sum((conj(y_1).*y_1),1) + sum((conj(y_2).*y_2),1))/(1+var(1,k));
        
        %temp2 = ((y_1.*repmat(var(1,k)+argm(i).^2,L(p),1))-(y_2.*repmat(argm(i),L(p),1)));
        temp2 = ((y_1.*(var(1,k)+argm(i).^2))-(y_2.*argm(i)));
        temp3 = sum(conj(temp2).*temp2,1);
        
        temp4 = ((y_2.*(var(1,k)+1))-(y_1.*argm(i)));
        temp5 = sum(conj(temp4).*temp4,1);
        
        temp6 = (temp3+temp5)/(var(1,k)*(var(1,k)+1+argm(i).^2));

        temp7 = sum(conj(h_hat).*h_hat,1);
        
        obj(i,:) = temp1+temp6+temp7;
      end
...
%%PLOTTING
legendtext = strcat('MAP joint chEst L= ',cellstr(num2str(L')));
if splot ~= 0
    figure(1)
    semilogy(SNR_db,SER,'o-');
    grid on
    axis([min(SNR_db) max(SNR_db) 1e-6 1])
    title('BPSK - SIMO(MRC) SER simulation with joint MAP channel estimate')
