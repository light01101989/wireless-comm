...
%% Coherent detection performed with ML channel estimates

%clear all;close all;
function [SER legendtext] = HW4_Q3_2(L,splot)
...
for p = 1:length(L)
    for k = 1:length(sd)
      ...
      %% ML channel estimate: h_hat = y_tr
      h_hat = y_tr;
...
%%PLOTTING
legendtext = strcat('ML chEst L= ',cellstr(num2str(L')));
if splot ~= 0
    figure(1)
    semilogy(SNR_db,SER,'o-');
    grid on
    axis([min(SNR_db) max(SNR_db) 1e-6 1])
    title('BPSK - SIMO(MRC) SER simulation with ML channel estimate')
