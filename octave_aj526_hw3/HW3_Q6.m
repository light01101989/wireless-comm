...
%% Email/Netid: aj526
%% Date: 09/24/2015
%% Monte-Carlo simulation for estimating SER of BPSK under MISO config

function [SER legendtext] = HW3_Q6(L)
% Find the SER for MISO case with L channel inputs(antennaes),
% Same symbol sent over all the TX antennas without any Beamforming
%
% Input:
...
SNR = 10.^(SNR_db/10);
sd = sqrt(L'./SNR);  % Standard Deviation
var = sd.^2;        % Variance
...
for p = 1:length(L)
    ...
    for k = 1:length(sd)
      ...
      %% RECEIVED SIGNAL ACCORDING TO RELATION y(1xT) = sum(h)x + n
      y = sum(h_channel,1).*x_bpsk + noise;
      ...
      y_new = dot(sum(h_channel,1)./norm(sum(h_channel,1),2,'columns'),y,1);    
...
title('BPSK - MISO: SER Simulation with varying TX antennae')
xlabel('signal-to-noise ratio (SNR) [dB]')
ylabel('symbol error rate (SER)')
legendtext ='';
for p = 1:length(L)
    legendtext = [legendtext; sprintf('Normal L = %d ',L(p))];
end
legend(legendtext)
